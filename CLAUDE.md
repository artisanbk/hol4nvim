...
- `lua/hol4nvim/` — the plugin. Layering (keep it): `transform.lua` is pure string→string reshapers with no plugin dependencies; `repl.lua` owns transport/session state and `M.config`, and generates the machine-agnostic external-session `$HOL_CONFIG` loader (`write_hol_config`) and the opt-in managed shell-rc export block (`write_shell_rc`/`splice_rc`, `$SHELL`-derived or `config.shell_rc`), both driven by `:HolExternalSetup` or, opt-in, by `setup()` itself via `auto_shell_rc`, so a hol you start yourself attaches to the fifo without a `~/.hol-config.sml`; `fifo.lua` is the fifo transport (never requires `repl` at load time); `init.lua` is `setup(opts)` + user commands; `keymaps.lua` is the single keymap registry; `health.lua` is `:checkhealth hol4nvim` (re-runs the resolvers, never duplicates their logic); `search.lua` is the theorem-database search (`hf`/`hm`), which captures results over the one-way transport via a temp file since `send()` has no return channel; `completion.lua` owns the completion vocabulary (static tactics + a DB-snapshot theorem cache, refreshed off the `HolReplStarted`/`HolLoaded` User autocmds `repl` fires) and `cmp.lua` is the thin nvim-cmp source over it.
