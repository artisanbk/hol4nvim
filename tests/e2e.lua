...
-- everything Vimhol-dependent below (pipe-routed multi-line sends, hc) then
...
-- hl : load deps from the open line; full HOLLoad shape also executes the
-- open itself quietly and prints the confirmation
goto_line("^open HolKernel")
repl.send_load_line()
wait_for("hl loads deps and confirms", "HOLLoad HolKernel", 60000)

-- hs : send a single line
goto_line("^val two")
repl.send_line()
wait_for("hs evaluates line", "val two = 2")

-- hs visual, ACTIVE visual mode: send while the selection is still live,
...
repl.send_visual()
wait_for("hs active-visual sends fun", "val triple = fn")

goto_line("^val it_should_be_nine")
repl.send_line()
wait_for("visual-sent fun works", "val it_should_be_nine = 9")

-- hu quiet send: binding evaluates but its printout is suppressed. The
...
wait_for("hu quiet value usable afterwards", "val quietly_check = 100")
t.check(
	"hu suppressed the binding printout",
	occurrences(term_text():gsub("\n", ""), "val quietly = 99") == 0
)

-- goal workflow: set goal via hs, prove via he
...
wait_for("he proves goal", "Initial goal proved")

goto_line("^proofManagerLib.drop")
repl.send_line()

-- hg : set the goal straight from the bare quotation line, prove, drop
goto_line("^`!a b\\. a + b")
repl.send_goal_line()
wait_for("hg sets goal", "Initial goal:", 30000, 2)

goto_line("^decide_tac")
repl.send_expand_line()
wait_for("he proves hg goal", "Initial goal proved", 30000, 2)

goto_line("^proofManagerLib.drop")
repl.send_line()

-- hS : with the sub_add goal set, split off `0 < n` as a subgoal
goto_line("^`!n\\. 1 <= n")
repl.send_goal_line()
wait_for("hg sets sub_add goal", "Initial goal:", 30000, 3)
...
-- select `0 < n` by decide_tac (without the leading >>), then hS;
-- like upstream, hS strips the trailing by-clause but not leading tokens.
...
wait_for("hS sets subgoal", "0 < n", 30000, 2)

-- hb backup undoes the sg: the arith goal is displayed again (3rd unicode
-- display: initial, post-strip_tac, now this)
repl.backup()
wait_for("hb backs up a step", "n − 1 + 1 = n", 30000, 3)

-- hp prints the current proof state: same goal, 4th display
repl.p()
wait_for("hp prints proof state", "n − 1 + 1 = n", 30000, 4)

-- hc reaches Vimhol's poller over the session pipe (no tactic is running,
-- so it reports the interrupt and that there is nothing to interrupt)
repl.interrupt()
wait_for("hc reaches Vimhol", "Vim interrupt")

-- he on the WHOLE `>> ... by ...` line: the leading >> is combinator noise
...
goto_line("^\\s*>> `0 < n` by decide_tac")
repl.send_expand_line()
wait_for("he strips >> and applies by-line", "0 < n", 30000, 4)

goto_line("^decide_tac")
repl.send_expand_line()
wait_for("he finishes sub_add proof", "Initial goal proved", 30000, 3)

goto_line("^proofManagerLib.drop")
repl.send_line()

-- hG unquoted goal: select the statement + Proof lines of a Theorem block
...
wait_for("hG sets unquoted goal", "Initial goal:", 30000, 4)

goto_line("^decide_tac")
repl.send_expand_line()
wait_for("he proves hG goal", "Initial goal proved", 30000, 4)
...
-- hc stops a runaway tactic (example section 7): the two-line rpt
...
repl.interrupt()
wait_for("hc stops the runaway tactic", "Vim interrupt", 30000, 2)
...
-- hs on a whole Theorem..QED block in active visual mode: the script-file
-- syntax goes through hol's filter and produces a thm binding
goto_line("^Theorem add_comm_test")
vim.cmd("normal! V4j")
repl.send_visual()
wait_for("hs sends Theorem block", "val add_comm_test =")
...
-- hy and hn display toggles (example section 8b): observable via how the
-- quotation results print; both toggled straight back
repl.toggle_types()
goto_line("^``1 + 2``")
repl.send_line()
wait_for("hy shows types", "(1 :num)")
repl.toggle_types()

repl.toggle_unicode()
goto_line("^``¬T``")
repl.send_line()
wait_for("hn prints ASCII negation", "~T")
repl.toggle_unicode()

-- he on a non-tactic: HOL errors, nvim survives
goto_line("^val this_is_not_a_tactic")
local ok = pcall(repl.send_expand_line)
t.check("he on non-tactic does not raise", ok)

-- h! send_document on the dedicated linear demo (examples/WholeScript.sml):
...
-- already loaded from the hl step above, as the demo's workflow instructs.
...
-- theorem-database search (search.lua): a name search runs DB.find in the
-- live session, which writes formatted hits to a temp file that the plugin
-- polls and renders in a scratch split. arithmeticTheory is in the base heap,
-- so "ASSOC" finds arithmetic.ADD_ASSOC.
local search = require("hol4nvim.search")
local sbuf = search.run("name", "ASSOC")
t.check("hf name search returns a panel buffer", sbuf ~= nil)
if sbuf then
	local lines = vim.api.nvim_buf_get_lines(sbuf, 0, -1, false)
	local stext = table.concat(lines, "\n")
	t.check("header denotes a name search", lines[1]:find("NAME", 1, true) ~= nil)
	t.check("search bar holds the typed query", lines[2] == "ASSOC")
	t.check("status line shows a hit count", stext:find("hits", 1, true) ~= nil)
	t.check("search results contain ADD_ASSOC", stext:find("ADD_ASSOC", 1, true) ~= nil)
end

-- completion (completion.lua): refresh snapshots the live DB into the cache
-- over the same temp-file round-trip; arithmeticTheory is in the base heap, so
-- the public name ADD_ASSOC must land among the completion items.
local completion = require("hol4nvim.completion")
local cdone, cok, ccount
completion.refresh(function(ok, info)
	cdone, cok, ccount = true, ok, info
end)
t.check(
	"completion refresh completes",
	vim.wait(20000, function()
		return cdone
	end, 200)
)
t.check("completion refresh succeeded", cok == true)
t.check("completion cache is populated", type(ccount) == "number" and ccount > 100)
do
	local found = false
	for _, it in ipairs(completion.cache.theorems) do
		if it.label == "ADD_ASSOC" then
			found = true
			break
		end
	end
	t.check("completion cache contains ADD_ASSOC", found)
end
t.check(
	"completion items merge tactics with the theorem cache",
	#completion.items() > #completion.cache.theorems
)

-- hX : close sends EOF and the job dies, pruning the session stack
repl.close()
local closed = vim.wait(15000, function()
	return repl.current() == nil
end, 200)
t.check("hX ends session and prunes stack", closed)
